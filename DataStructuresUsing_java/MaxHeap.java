public class MaxHeap{
    ...
    private void rootDeleteHelper(int position){
        int swap;
        if(arr[position]<maxValue(position)){
            swap =arr[position];
            arr[position] = arr[maxValue(position)];
            arr[maxValue(position)] = swap;
            rootDeleteHelper(maxValue(position));

        }

    }
    private int maxValue(int position){
